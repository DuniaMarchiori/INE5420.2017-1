...
public: 
	...
	Coordenada* transformaViewport(Coordenada* ponto, Window* wind) {
		...
		transformada->x = ( ( (ponto->x - wMin->x)/(wMax->x - wMin->x) ) * (getViewportMax()->x - viewportMin->x) );
		transformada->y = ( ( 1 - ( (ponto->y - wMin->y)/(wMax->y - wMin->y) ) ) * (getViewportMax()->y - viewportMin->y) );
		return transformada;
	}
};
