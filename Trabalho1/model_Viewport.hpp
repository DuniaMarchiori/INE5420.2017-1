...
public:
	...
	*/
	Coordenada* transformaViewport(Coordenada* ponto, Coordenada* vpMax) {
		...
		transformada->setX( ( ( (ponto->getX() - wMin->getX())/(wMax->getX() - wMin->getX()) ) * ((vpMax->getX() - margem) - vpMin->getX()) ) + vpMin->getX() );
		transformada->setY( ( ( 1 - ( (ponto->getY() - wMin->getY())/(wMax->getY() - wMin->getY()) ) ) * ((vpMax->getY() - margem) - vpMin->getY()) ) + vpMin->getY() );
		return transformada;
	}
};
